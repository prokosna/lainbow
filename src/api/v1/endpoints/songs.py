...
from api.core import text_embedding
from api.session_manager_wrap import get_db_session
from domain import milvus_utils
from domain.schemas import EmbeddingModel, Song, SongEmbedding, SongFeatures
from fastapi import APIRouter, Depends, HTTPException, Query
from pydantic import BaseModel
from sqlmodel import Session, col, select
from worker import acoustic_features, clap, mert
...
def find_similar_songs(
    ...
    model_name: EmbeddingModel = Query(  # noqa: B008
        default=EmbeddingModel.ACOUSTIC_FEATURES,
...
def search_songs_by_natural_language(
    q: str = Query(..., description="Natural language query for the song search."),
    limit: int = Query(default=10),
    db: Session = Depends(get_db_session),  # noqa: B008
) -> SearchSongResult:
    """
    Searches for songs based on a natural language query using the CLAP model.
    """
    try:
        query_vec = text_embedding.get_text_embedding(q)
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to generate text embedding: {e}"
        ) from e
