...
MODEL_TYPE = "muq"
COLLECTION_NAME = "muq_audio_embeddings"
...
def delete_vectors(file_paths: list[str]) -> None:
    """
    Deletes vectors from the MuQ Milvus collection based on their file paths.
    """
    if not file_paths:
...
def get_audio_embeddings_batch(audio_paths: list[Path]) -> dict[str, np.ndarray]:
    """
    Computes L2-normalized MuQ audio embeddings for a batch of audio files.
    Audio is split into chunks, and the embeddings for each chunk are averaged.
    """
    if not audio_paths:
        return {}
    ...
    for audio_path in audio_paths:
        try:
            ...
            logger.error(f"Failed to process {audio_path} for MuQ, skipping: {e}")
...
def get_audio_embedding(audio_path: Path) -> np.ndarray:
    """
    Computes an L2-normalized MuQ audio embedding for a single audio file.
    """
    results = get_audio_embeddings_batch([audio_path])
    path_str = str(audio_path)

    if path_str not in results:
        raise Exception(f"Failed to get MuQ embedding for {audio_path}")
