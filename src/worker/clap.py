...
COLLECTION_NAME = "clap_audio_embeddings"
...
def delete_vectors(file_paths: list[str]) -> None:
    """
    Deletes vectors from the CLAP Milvus collection based on their file paths.
    """
    if not file_paths:
...
def get_audio_embeddings_batch(audio_paths: list[Path]) -> dict[str, np.ndarray]:
    """
    Computes L2-normalized audio embeddings for a batch of audio files.

    This function processes multiple audio files efficiently by batching all their
    segments together for a single pass through the CLAP model.

    Args:
        audio_paths: A list of paths to the audio files.

    Returns:
        A dictionary mapping file paths (as strings) to their L2-normalized
        numpy array embeddings. Files that fail to process are omitted from the result.
    """
    if not audio_paths:
        return {}
    ...
    for audio_path in audio_paths:
        try:
            audio_file = audio.load_audio_for_librosa(audio_path, sr=CLAP_SAMPLING_RATE)
            audio_data, sr = librosa.load(audio_file, sr=CLAP_SAMPLING_RATE, mono=True)
    ...
    try:
        ...
        raise Exception(f"Failed to get CLAP embeddings for batch: {e}") from e


def get_audio_embedding(audio_path: Path) -> np.ndarray:
    """
    Computes a L2-normalized audio embedding for a given audio file.
    The normalized embedding is ready to be stored in Milvus for cosine similarity search.

    This function extracts 4 segments of 10 seconds each from the audio,
    excludes the last segment (to avoid end-of-song effects like silence/fade-out),
    and computes the average embedding of the remaining 3 segments.
    ...
    """
    """Computes an L2-normalized audio embedding for a single audio file.

    This is a convenience wrapper around the batch-processing `get_audio_embeddings_batch` function.
    """
    results = get_audio_embeddings_batch([audio_path])
    path_str = str(audio_path)

    if path_str not in results:
        raise Exception(f"Failed to get embedding for {audio_path}")
